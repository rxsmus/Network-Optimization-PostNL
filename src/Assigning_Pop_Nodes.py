...
def assign_population_nonsqr(df, service_points):
    # Iterate over service points
    for sp_id, sp_info in service_points.items():
        total_demand = sp_info['population']
        current_population = df[df['nearest_service_point'] == sp_id]['assigned_population'].sum()
        remaining_demand = total_demand - current_population

        if remaining_demand <= 0:
            continue  # If demand is already met or exceeded, skip to the next service point

        # Find nodes served by this service point with nothing in the "square" field
        unserved_nodes = df[(df['nearest_service_point'] == sp_id) & (df['assigned_population'] == 0) & df['square'].isnull()]

        if unserved_nodes.empty:
            continue  # If no unserved nodes, skip to the next service point

        # Distribute remaining demand among unserved nodes
        demand_per_node = remaining_demand / len(unserved_nodes)
        # Update assigned population for unserved nodes
        df.loc[unserved_nodes.index, 'assigned_population'] = demand_per_node

    # Ensure no negative assigned_population
    df['assigned_population'] = df['assigned_population'].clip(lower=0)
...
def main():
    ...
    df['population'] = df['population'].replace(2, 0) # going back to no-pop squares
    ...
    # Assign population to nodes not in squares
    df = assign_population_nonsqr(df, service_points)

## Printing and csv saving
    # Save the updated DataFrame to a new CSV file
    df.to_csv('pop_assigned_0square-basis.csv', index=False)
